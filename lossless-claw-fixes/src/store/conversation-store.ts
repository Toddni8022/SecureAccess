...
export class ConversationStore {
  ...
  private searchRegex(
    ...
    // Cap the pre-filter scan to limit ReDoS exposure (#76): a malicious regex
    // applied to an unbounded row set could pin the CPU indefinitely.
    const MAX_SCAN_ROWS = Math.max(limit * 20, 500);
    ...
    const rows = this.db
      ...
      .all(...args, MAX_SCAN_ROWS) as unknown as MessageRow[];
