...
> **Note:** `go install github.com/Martian-Engineering/lossless-claw/tui@latest` will produce a
> binary named `tui` (after the directory name), not `lcm-tui`. Use `go build -o lcm-tui .`
> to get the correctly-named binary, or rename it after installation:
> ```bash
> go install github.com/Martian-Engineering/lossless-claw/tui@latest
> mv "$(go env GOPATH)/bin/tui" "$(go env GOPATH)/bin/lcm-tui"
> ```
